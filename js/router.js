...
async function launchController(controllerName) {
    ...
    try {
        ...
        console.log("before module.init");
        module.init();
    } catch (error) {
        console.log(error);
    }
...
function setCurrentRoute({ path, controller }) {
    ...
    console.log("currentPath",routes.currentPath.path);
    console.log("currentPath",routes.currentPath.controller);
}
...
function navigate(path, firstLoad = false) {
    ...
    console.log("launch",launchController(route.controller));
    launchController(route.controller);
}
