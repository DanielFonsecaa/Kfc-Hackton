...
function render(athletes){
    ...
    .map(competition => {
        if (competition[0].competitionName === '100m female'){
           return competition[1].time;
            
        }
       return competition[0].time
    })
    ...
    heading.id = 'heading';
    heading.innerHTML = '<h1>200 Meters Sprint Relay Women</h1>';
    ...
    startButton.addEventListener("click", () => {
        ...
        ids = Array.from(boxes).map((box, index) =>
            setInterval(() => move(box, ids[index]), speeds[index] * Math.random() * (4.16 - 2.0) + 2.0)
        );
